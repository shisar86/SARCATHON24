...
# Load a fast sentence transformer model (optimized for speed)
model = SentenceTransformer('sentence-transformers/all-MiniLM-L6-v2')  # Fast and optimized model
...
# Create an FAQ index using FAISS for fast similarity search
def create_faq_index(faq_data):
    faq_questions = []
    faq_answers = []
    for category, faqs in faq_data.items():
        for faq in faqs:
            faq_questions.append(faq['question'])
            faq_answers.append(faq['answer'])
    
    # Encode all FAQ questions to get their embeddings
    faq_embeddings = model.encode(faq_questions, convert_to_tensor=True)
    
    # Convert embeddings to numpy array and initialize FAISS index
    faq_embeddings_np = faq_embeddings.cpu().numpy()
    dim = faq_embeddings_np.shape[1]  # Dimension of embeddings
    index = faiss.IndexFlatL2(dim)  # L2 distance for semantic similarity
    index.add(faq_embeddings_np)  # Add embeddings to FAISS index

    return index, faq_questions, faq_answers

# Initialize the FAISS index
index, faq_questions, faq_answers = create_faq_index(faq_data)

# Synchronous function to find the most similar FAQs to the query
...
# Define the search route in Flask
@app.route('/search', methods=['POST'])
def search_faq():
    data = request.json
    query = data['query']

    # Find similar FAQs synchronously
